#!/usr/bin/env python3
"""
Triple Monitor Light Show - Shows the same visualization on each of 3 monitors
Ports the original HTML/Canvas animation to Python with full visual effects
"""
...
class LightShow:
    ...
    def draw_fractal_tendril(self, x, y, angle, length, intensity, depth, hue_offset):
        ...
        try:
            ...
            # Actually draw with alpha
            if depth > 1:
                width = max(1, depth)
            else:
                width = 1

            # Draw organically curved line
            num_segments = 5
    ...
    def draw_funky_waveforms(self, freq_data, bass, mid, treble, average):
        ...
        if len(points_top) > 2:
            ...
            color = self.hsv_to_rgb(hue, 0.6, 0.3)  # Much more subtle
            # Thinner lines
            pygame.draw.lines(self.screen, color, False, points_top, 1)
            pygame.draw.lines(self.screen, color, False, points_bottom, 1)
        ...
        if len(points) > 2:
            # Subtle gradient - more transparent
            for i in range(len(points) - 1):
                hue = (i / len(points) * 360 + self.time * 120) % 360
                color = self.hsv_to_rgb(hue, 0.5, 0.25)  # Very subtle
                pygame.draw.line(self.screen, color, points[i], points[i + 1], 1)  # Thin line
        ...
        for i in range(num_tendrils):
            ...
            color = self.hsv_to_rgb(hue, 0.4, 0.2 + intensity * 0.2)  # Much more transparent
            pygame.draw.line(self.screen, color, (int(x1), int(y1)), (int(x2), int(y2)), 1)  # Thin
...
if __name__ == "__main__":
    try:
        print("Starting triple monitor light show...")
        print("Each monitor will show the full visualization")
